#!/usr/bin/env python3
"""Official XSched benchmark for Lv1/Lv2 preemption latency scaling.

This is a direct restoration of the earlier mechanism-level CUDA benchmark style:
it uses NVRTC at runtime to compile a fixed-duration sleep kernel, continuously
launches commands on an XQueue, and measures suspend/resume latency under Lv1/Lv2.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the official XSched Lv1/Lv2 preemption latency scaling benchmark."
    )
    p.add_argument("--levels", type=int, nargs="+", default=[1, 2], choices=[1, 2],
                   help="Preemption levels to test.")
    p.add_argument("--arch", default="89",
                   help="CUDA compute capability used for NVRTC, e.g. 89 for RTX4060.")
    p.add_argument("--kernel-us", type=int, nargs="+",
                   default=[50, 100, 250, 500, 1000, 2000],
                   help="Per-command kernel durations in microseconds.")
    p.add_argument("--threshold", type=int, default=8)
    p.add_argument("--batch-size", type=int, default=4)
    p.add_argument("--burst", type=int, default=64,
                   help="How many commands each launcher burst submits before a synchronize.")
    p.add_argument("--iters", type=int, default=40,
                   help="Measured suspend/resume samples per case.")
    p.add_argument("--warmup-s", type=float, default=1.0)
    p.add_argument("--jitter-us", type=int, default=800)
...
def make_result_dir(user_dir: Path | None) -> Path:
    if user_dir is not None:
        ...
        out = RESULTS / f"preemption_latency_scaling_{time.strftime('%Y%m%d_%H%M%S')}"
...
def find_nvrtc() -> str:
    candidates = [
        ...
        Path("/usr/lib/x86_64-linux-gnu/libnvrtc.so"),
    ]
    found = find_existing(candidates)
    return str(found) if found else "libnvrtc.so"


def find_libs() -> tuple[Path, Path, Path]:
    preempt = find_existing([
        ROOT / "output/lib/libpreempt.so",
        ROOT / "build/preempt/libpreempt.so",
        ROOT / "build_cuda_test/preempt/libpreempt.so",
    ])
    hal = find_existing([
        ROOT / "output/lib/libhalrtx4060.so",
        ROOT / "build/platforms/RTX4060/libhalrtx4060.so",
        ROOT / "build_cuda_test/platforms/RTX4060/libhalrtx4060.so",
    ])
    shim = find_existing([
        ROOT / "output/lib/libshimrtx4060.so",
        ROOT / "build/platforms/RTX4060/libshimrtx4060.so",
        ROOT / "build_cuda_test/platforms/RTX4060/libshimrtx4060.so",
    ])
    if not preempt or not hal or not shim:
        raise SystemExit("missing XSched runtime libraries; build/output libs are required.")
    return preempt, hal, shim
...
def load_libs(real_libcuda: str):
    os.environ["XSCHED_CUDA_LIB"] = real_libcuda
    os.environ["CUXTRA_CUDA_LIB"] = real_libcuda
    os.environ.setdefault("XSCHED_CUDA_MEM_OVERSUB", "0")

    lib_preempt, lib_hal, lib_shim = find_libs()
...
def compile_ptx(nvrtc, arch: str) -> bytes:
    ...
    check_nvrtc(
        nvrtc.nvrtcCreateProgram(ctypes.byref(prog), KERNEL_SRC.encode(), b"sleep.cu", 0, None, None),
        "nvrtcCreateProgram",
    )
...
def launch_sleep(cuda, func, stream, duration_ns: int):
    ...
    check_cuda(
        cuda.cuLaunchKernel(func, 64, 1, 1, 64, 1, 1, 0, stream, params, None),
        "cuLaunchKernel(sleep_ns)",
    )


def run_case(real_libcuda: str, arch: str, level: int, kernel_us: int, threshold: int,
             batch_size: int, burst: int, iters: int, warmup_s: float,
             jitter_us: int) -> dict[str, object]:
    preempt, hal, cuda, nvrtc = load_libs(real_libcuda)
    setup(preempt, hal, cuda, nvrtc)
    ctx = create_context(cuda)
    ptx = compile_ptx(nvrtc, arch)
    module, func = load_kernel(cuda, ptx)
    stream, xq = create_xqueue(preempt, hal, cuda, level, threshold, batch_size)
        ...
        try:
            ...
            while not stop.is_set():
                for _ in range(burst):
                    launch_sleep(cuda, func, stream, kernel_us * 1000)
    ...
    preempt_us: list[float] = []
    restore_us: list[float] = []
    try:
        for _ in range(iters):
            time.sleep(random.uniform(0, jitter_us) / 1_000_000.0)
            ...
            time.sleep(random.uniform(jitter_us, jitter_us * 2) / 1_000_000.0)
            start = time.perf_counter_ns()
...
def write_csv(path: Path, rows: list[dict[str, object]]) -> None:
    with path.open("w", newline="") as f:
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({k: v for k, v in row.items() if not k.startswith("raw_")})
...
def main() -> int:
    ...
    for level in args.levels:
        for kernel_us in args.kernel_us:
            print(f"running level={level} kernel_us={kernel_us}", flush=True)
            row = run_case(
                real_libcuda=args.real_libcuda,
                arch=args.arch,
                level=level,
                kernel_us=kernel_us,
    ...
    rows.sort(key=lambda r: (int(r["level"]), int(r["kernel_us"])))
    write_csv(result_dir / "preemption_latency_scaling.csv", rows)
    summary = {
        "benchmark": "xsched_official_preemption_latency_scaling",
        "levels": args.levels,
    ...
    print(f"csv: {result_dir / 'preemption_latency_scaling.csv'}")
